...
func main() {
	...
	if len(os.Args) > 1 {
		...
		resp, detail, err := plan_execute_replan.ResumePersistentPlanAgent(ctx, executionID)
		if err != nil {
			panic(err)
		}
		fmt.Println("----- Resume Response -----")
		fmt.Println(resp)
		fmt.Println("----- Resume detail -----")
		fmt.Println(detail)
		return
	}
	query := `
...
"6. 分别将告警对应查询到的信息进行总结分析,最后汇总所有告警和总结。"`
	resp, detail, err := plan_execute_replan.BuildPersistentPlanAgent(ctx, query)
	if err != nil {
		panic(err)
	}
